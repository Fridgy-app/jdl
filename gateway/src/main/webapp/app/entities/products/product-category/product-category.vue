<template>
  <div>
    <h2 id="page-heading" data-cy="ProductCategoryHeading">
      <span v-text="$t('gatewayApp.productsProductCategory.home.title')" id="product-category-heading">Product Categories</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('gatewayApp.productsProductCategory.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'ProductCategoryCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('gatewayApp.productsProductCategory.home.createLabel')"> Create a new Product Category </span>
          </button>
        </router-link>
      </div>
    </h2>
        ...
        <form name="searchForm" class="form-inline" v-on:submit.prevent="search(currentSearch)">
          <div class="input-group w-100 mt-3">
            <input
              ...
              name="currentSearch"
              id="currentSearch"
              v-bind:placeholder="$t('gatewayApp.productsProductCategory.home.search')"
    ...
    <div class="alert alert-warning" v-if="!isFetching && productCategories && productCategories.length === 0">
      <span v-text="$t('gatewayApp.productsProductCategory.home.notFound')">No productCategories found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="productCategories">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="$t('gatewayApp.productsProductCategory.name')">Name</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span
          id="gatewayApp.productsProductCategory.delete.question"
      ...
      <div class="modal-body">
        <p id="jhi-delete-productCategory-heading" v-text="$t('gatewayApp.productsProductCategory.delete.question', { id: removeId })">
