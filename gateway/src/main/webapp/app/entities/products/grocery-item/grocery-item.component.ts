...
export default class GroceryItem extends mixins(JhiDataUtils) {
  ...
  public groceryItems: IGroceryItem[] = [];

  public isFetching = false;

  public mounted(): void {
    this.retrieveAllGroceryItems();
  }
  ...
  public retrieveAllGroceryItems(): void {
          ...
          res => {
            if (res.data && res.data.length > 0) {
              for (let i = 0; i < res.data.length; i++) {
                this.groceryItems.push(res.data[i]);
              }
  ...
  public prepareRemove(instance: IGroceryItem): void {
  ...
  public removeGroceryItem(): void {
    this.groceryItemService()
      .delete(this.removeId)
      .then(() => {
        const message = this.$t('productsApp.productsGroceryItem.deleted', { param: this.removeId });
