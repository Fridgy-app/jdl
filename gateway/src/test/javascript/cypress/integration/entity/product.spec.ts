...
describe('Product e2e test', () => {
  ...
  beforeEach(() => {
    ...
    cy.intercept('GET', '/services/products/api/products*').as('entitiesRequest');
    cy.visit('');
  ...
  it('should create an instance of Product', () => {
    ...
    cy.intercept('GET', '/services/products/api/products*').as('entitiesRequestAfterCreate');
    cy.visit('/');
    cy.clickOnEntityMenuItem('product');
  ...
  it('should delete last instance of Product', () => {
    cy.intercept('GET', '/services/products/api/products*').as('entitiesRequest');
    cy.intercept('DELETE', '/services/products/api/products/*').as('deleteEntityRequest');
    cy.visit('/');
    cy.clickOnEntityMenuItem('product');
    cy.wait('@entitiesRequest').then(({ request, response }) => {
      ...
      if (startingEntitiesCount > 0) {
        ...
        cy.intercept('GET', '/services/products/api/products*').as('entitiesRequestAfterDelete');
        cy.visit('/');
        cy.clickOnEntityMenuItem('product');
